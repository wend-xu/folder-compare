...
- 本轮执行 `Phase 17C`，在 `Phase 16A + 16A fix-1 + 16B + 16C + 16C fix-1 + 17A + 17A fix-1 + 17B + 17B fix-1` 的稳定基线上，完成一轮低风险 UI 收口：落地历史 workbench bug `B/C/D`，并把 `Compare Inputs` 的主动作交互从“按钮 + 右侧说明文字”收口为“主动作按钮 + 轻量 tooltip + Compare Status summary-first”；不引入新的 compare/core 架构、settings framework、tree mode 或 explanation-heavy hover system。
- 已完成并关闭：
  - `Phase 17C`
  - `Phase 17B fix-1`
  - `Phase 17B`
...
- 当前稳定基线：
  ...
  - workspace 外层双卡片感已收口：外层 workspace wrapper 现为透明视觉壳，workbench host inset 更紧，workspace loading mask 也已挂到同一 workbench surface
...
- 为什么下一步才是剩余 `Phase 17`：
  ...
  - 因此下一线程应继续剩余 `Phase 17` 工作，优先单独处理 `docs/ui-bug-root-cause-and-fix-plan-2026-03.md` 中仍未关闭的 `A. Diff shell state card still feels oversized, accent-heavy, and visually misaligned`，而不是继续重开 `15.3A` 到 `15.8 fix-1`、`16A` 到 `16C fix-1`、或 edition 兼容修复。

## 快照（Snapshot）

- 日期：`2026-03-21`（Asia/Shanghai）
- 分支：当前已完成 `Phase 17C`
- 工作区：`fc-ui-slint` Compare Inputs 主动作收口、workbench bug `B/C/D` 修复、主文档同步改动
- 最近提交：
...
1. `Phase 17C` 已完成；Sidebar 仍然保持 `Compare Inputs -> Compare Status -> Filter / Scope -> Results / Navigator`。
...
4. compare / tooltip / workbench 当前补充 contract：
   - `Compare` 现已是与输入列对齐的 full-width primary action lane；`Browse` 与路径输入布局不变
...
6. 后续线程继续 `Phase 17` 剩余子项时，不要重开 `Phase 15.3A` 到 `Phase 15.8 fix-1`、`Phase 16A`、`16A fix-1`、`16B`、`16C`、`16C fix-1`、`Phase 17A`、`Phase 17A fix-1`、`Phase 17B`、`Phase 17B fix-1`、`Phase 17C`，也不要重开独立 workspace `edition = "2024"` 里程碑。
7. 下一线程若继续 UI bug 收口，应优先单独落地 `A. Diff shell state card still feels oversized, accent-heavy, and visually misaligned`，而不是重新扩张到 `B/C/D` 或 Compare Inputs。
...
- In Scope：
  - 剩余 `Phase 17`
  - `DiffStateShell` embedded visual 收口
...
- UI / shell：
  ...
  - `Compare` 主动作现已与输入列对齐为独占 action lane；它不再与右侧说明文字共享空间
  ...
  - workspace 外层 wrapper 当前已退为透明视觉壳，workbench host inset 已收紧，workspace loading mask 与用户感知的 workbench surface 保持同一边界
  ...
  - `A. Diff shell state card still feels oversized, accent-heavy, and visually misaligned` 仍未修复；后续若继续 `Phase 17`，应把它视为优先的独立视觉收口项
...
- 唯一建议的下一步是继续剩余 `Phase 17` 工作，优先单独处理 bug 文档中的 `A`。
- 后续实现应建立在当前 `0.2.18 + edition 2024 + rust 1.94.0 + slint 1.15.1 + Phase 16A + 16A fix-1 + 16B + 16C + 16C fix-1 + Phase 17A + Phase 17A fix-1 + Phase 17B + Phase 17B fix-1 + Phase 17C` 基线上，不重开升级、edition 迁移或 `Phase 15` closeout。
...
- 本轮验证重点是 `Phase 17C` 的 Compare Inputs 主动作收口、workbench bug `B/C/D` 修复，以及主文档同步：
