...
- 轮次定义：`Dependency upgrade executed through Phase 15.5 fix-3; Phase 15.6 cleanup is the next default target`。
- 改了什么：
  - 实际完成 `Phase 15.3A`、`Phase 15.3B`、`Phase 15.4`、`Phase 15.5`，并在正式进入 `Phase 15.6` 之前补做 `Phase 15.5 fix-1`、`Phase 15.5 fix-2`、`Phase 15.5 fix-3`；
...
- 为什么影响下一线程：如果下一线程仍按旧假设把 `15.2E` 当作 deferred、重新执行 `15.5`，或者继续把当前基线当作 `slint = 1.8.0`，会重复做已完成工作并误判 `15.6` 的起点；如果忽略 `15.5 fix-1` / `fix-2`，很容易把 `Workspace Diff detail line` 的方框误判成 `fc-core` 编码问题，或者把当前 `UiTypography` 收敛重新退回多层 prop threading；如果忽略 `15.5 fix-3`，则容易继续在升级后的 `ListView` 路径上追横向滚动条问题，反复触发同类回归；如果跳过 `15.6` 直接做 `Phase 16`，会再次把同步清理、导航增强、残留输入 affordance 评估混成一轮。
- 保持不变：`15.2D` 的 IA 与 shell contract 不变；`Diff/Analysis` shell、connected tabs、loading scope boundary、`SelectableSectionText` / `SelectableDiffText` 的可选中文本边界、modal draft 行为、以及本地 `toast/loading/menu` controller 边界均不改；本轮只修复 `Diff detail` 横向滚动结构、不扩张右键菜单范围；workspace `edition` 仍是 `2021`；UI 仍使用内联 `slint::slint!`，`50ms` 轮询仍保留并留待 `Phase 15.6` 处理。

## 快照（Snapshot）

- 日期：2026-03-18（Asia/Shanghai）
- 分支：`dev-phase15_3_to_6_upgrade_plan`
- 工作区：有改动（`Phase 15.5 fix-3` 已执行，代码与三份主文档已同步到 `0.2.17` 基线）
- 最近提交：
  - `3b13629` phase 15.5
  - `e8bb75a` Phase 15.3A / 15.3B / 15.4 doc sync
  - `c90f746` Phase 15.3A / 15.3B / 15.4
- 当前架构基线：`docs/architecture.md`（`15.2E` 已在 `rust 1.94.0 + slint 1.15.1` 基线上落地，且 `Phase 15.5 fix-1` 已修复 glyph fallback 回归、`Phase 15.5 fix-2` 已把实现收敛到 `UiTypography`、`Phase 15.5 fix-3` 已恢复 `Diff detail` 横向滚动条；下一默认执行目标是 `Phase 15.6`）

## 当前目标（Execution Focus）

1. 以 `Phase 15.5`、`Phase 15.5 fix-1`、`Phase 15.5 fix-2`、`Phase 15.5 fix-3` 已稳定为前提，下一步默认执行 `Phase 15.6`：清理 `50ms` 轮询与 model churn。
2. 保持 phase train 分层：`Phase 15.6` -> `Phase 15.7`（optional）-> `Phase 16`，不回头重做 `15.3A/15.3B/15.4/15.5`，也不跳过 `15.6` 直接做导航增强。
...
- In Scope：
  - 以 `rust 1.94.0 + slint 1.15.1 + 15.5/15.5 fix-1/15.5 fix-2/15.5 fix-3 已完成` 为基线，推进 `Phase 15.6`
  - 后续阶段执行时同步更新三份主文档
...
6. 依赖升级路线已完成到 `Phase 15.5 fix-3`；后续必须按 `Phase 15.6` -> `Phase 15.7`（optional）-> `Phase 16` 推进，并在同一轮同步主文档。
...
- Now：
  - `Phase 15.3A` / `15.3B` / `15.4` / `15.5` 已完成并通过验证，`Phase 15.5 fix-1` 已修复 `Workspace Diff detail line` 的 glyph fallback 回归，`Phase 15.5 fix-2` 已把实现收敛到共享 `UiTypography` token，`Phase 15.5 fix-3` 已恢复 `Diff detail` 横向滚动条并补齐尾行 safe spacer
  - 当前稳定基线是 `15.2E` 已落地 + `rust 1.94.0 + slint 1.15.1 + workspace version 0.2.17`
  - 下一默认工作是 `Phase 15.6`
- Next：
...
3. `Phase 15.6` 不得回退 `Phase 15.5` 已落地的原生 editable-input surface；不要把输入菜单重新改回 overlay 拦截、私有事件链路或自写编辑逻辑。
...
7. 不要在 `Phase 15.6` 清理中移除 `UiTypography.selectable_content_font_family` 当前的 glyph fallback 收敛，除非已经用真实 mixed Latin+CJK 文本验证过 `slint 1.15.x` 默认 `TextInput` 路径不再把全角标点渲染成方框。
8. 不要把 `Diff detail` 的显式 `ScrollView` 再回退到升级后的 `ListView` 路径，除非已经用真实长行样本验证横向滚动条与尾行可选中性都稳定。
9. 不要把 `Phase 15.6`、`Phase 16`、以及残留输入 affordance 评估混成同一轮；也不要重新开临时 checklist 文档绕开主文档同步。
...
> 把 `Phase 15.3A`、`Phase 15.3B`、`Phase 15.4`、`Phase 15.5`、`Phase 15.5 fix-1`、`Phase 15.5 fix-2`、`Phase 15.5 fix-3` 视为已完成。  
> 把当前基线视为：`15.2E` 已在 `rust 1.94.0 + slint 1.15.1` 上落地，且 read-only selectable content 的 glyph fallback 回归已修复并收敛到共享 `UiTypography` token，`Diff detail` 横向滚动条也已切到显式 `ScrollView` 路径恢复稳定。  
> 下一步默认从 `Phase 15.6` 开始，不要回头重做升级或输入菜单补票，也不要直接推进 `Phase 16`。  
> 保持当前 IA 与 phase 边界。  
> 不要回退 Diff/tabs/Analysis shell 收敛结果，也不要把本地 toast/loading/menu controller 重新塞进 `AppState/Presenter`。  
> 不要把 `Phase 15.6`、`Phase 16` 和残留输入 affordance 评估混在同一轮。  
...
> 不要在 `Phase 15.6` 里顺手推进 `Phase 15.7` 菜单美化或 `SelectableDiffText` 行级右键菜单，它们都必须保持独立范围。  
