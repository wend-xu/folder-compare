...
- 已知上一轮 `19B` GUI smoke 已人工确认；当前 `19C` 增量仍应重新做人工 smoke：
  - sidebar hide / restore
  - Compare View 与 File View 在 sidebar visible/hidden 下的头部与几何
  - Compare -> File -> Back 闭环在 sidebar visible/hidden 下是否仍成立
- 字体链路后续收口阶段另已验证：
...
- 当前真实焦点是：
  ...
  - 把 `Phase 19A`、`19B fix-2`、`19C` 视为已实现事实
  - 后续如有新线程，默认应从 `19C` 已成立 shell contract 出发，而不是重复 `fix-1`、`19B` 收口，或把 `19C` 写回 proposal
  - 除非遇到新回归，否则不要继续把 `18C fix-*` 当作默认主线
  - 不要回退到 “可视区域/locate 仍 deferred” 的旧叙事
  - 不要把 compare workspace 的长期数据基础重新拉回到 `entry_rows` 字符串链路
  - 不要把 `19C` 顺手扩张成隐藏版 `19D / 19E`
...
- 不要把 landed `19C` 顺手扩写成隐藏版 `19D / 19E`。
...
- 不要把 `19D / 19E` 的 richer compare surface、目录详情、dual-tree、horizontal scrolling 或 compare-core 变更写成已实现事实。
...
- deeper Compare View / File View redesign beyond the current shell closeout
